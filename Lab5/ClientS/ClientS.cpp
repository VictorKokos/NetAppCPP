...
{
    // Получаем IP-адрес сервера по его символическому имени
    ...
    serv.sin_port = ((SOCKADDR_IN*)from)->sin_port; // Используем порт из параметра from
    serv.sin_addr.s_addr = *(u_long*)host->h_addr_list[0];
   

    cout << from->sa_family;
    // Отправляем позывной серверу
    ...
    // Проверяем совпадение позывного
...
int main()
{
    ...
    try
    {
        ...
        SOCKADDR_IN serv;
        serv.sin_family = AF_INET;
        serv.sin_port = htons(2008);


        char obuf[50] = "Hello";
        int lobuf = strlen(obuf);
        int flen = sizeof(serv);


        char serverName[] = "DESKTOP-DJ6A5P8";
        if (GetServerByName(serverName, obuf, (struct sockaddr*)&serv, &flen))
        {
            cout << "Answer from server: " << obuf << endl;


            cout << "Server address: " << inet_ntoa(serv.sin_addr) << ":" << ntohs(serv.sin_port) << endl;
        }
        else
        {
